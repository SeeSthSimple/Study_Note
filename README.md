...
- 如果目标是服务嵌入式工作衔接，目录组织优先围绕稳定工作主线展开：`C_C++`、`硬件`、`外设`、`通信协议`、`RTOS`、`Linux`、`驱动开发`、`板级与启动`、`工具链与调试`、`项目与架构`。
...
- 如果主题主线是外设本身的原理、初始化、时序、调试和迁移，优先放入 `外设`；如果主线是 Linux 框架或更底层硬件本质，则仍放在 `Linux` 或 `硬件`。
